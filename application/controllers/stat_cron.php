...
class stat_cron extends CI_Controller {
...
public function update_stats(){
	...
	public function stats($month='') { 
	 ...
	 $data['get_profile_views_company'] = $this->stat->get_profile_views_company($data['get_companyid_from_user']);
	 $data['get_advert_views_company'] = $this->stat->get_advert_views_company($data['get_companyid_from_user']);
	 $data['get_advert_views_month'] = $this->stat->get_advert_views_month($data['get_companyid_from_user'],$month);
	 $data['get_profile_views_month'] = $this->stat->get_profile_views_month($data['get_companyid_from_user'],$month);
	 $data['get_people_reached_month'] = $this->stat->get_people_reached_month($data['get_companyid_from_user'],$month);
	 $data['get_returned_visits'] = $this->stat->get_returned_visits($month);
	
	 
	 
	 
		 
		$data['tests'] = $this->stat->get_country_visits($month);
		$data['get_agent_id'] = 3;
	//data['get_return_visits'] = $this->stat->get_return_visits($month,$data['get_agent_id']);  

		
		 
	 $data['get_ads_shown'] = $this->stat->get_ads_shown($data['get_companyid_from_user']);
	 $data['get_pcs_reached'] = $this->stat->get_pcs_reached($data['get_companyid_from_user']);
	 $data['get_new_visits'] = $this->stat->get_new_visits();
										  
	 $data['get_chrome_visits'] = $this->stat->get_chrome_visits();
	 $data['get_mozilla_visits'] = $this->stat->get_mozilla_visits();
	 $data['get_safari_visits'] = $this->stat->get_safari_visits();
	 $data['get_ie_visits'] = $this->stat->get_ie_visits();
