...
$query45 = $this->db->query("SELECT id FROM companies where year(created_at) > ".$date." or year(updated_at) > ".$date." order by id desc");
...
$query = $this->db->query("SELECT company_id,website_id FROM website_company where website_id = ".WEBSITE_ID."  and company_id = ".$row45['id']." ");
		...
		foreach($query->result_array() as $row) {
			...
			$company_id ="706894"; //$row['company_id'];
 ...
 $d1=strtotime("10:21am August 11 2018");				
	...
	,'timestamp'=>date("Y-m-d h:i:sa", $d1)
	 ...
	 $res = $this->db2->insert('stats_ad_clicked', $data); 
			...
			//for adverts shown
			for ($x = 1; $x <= $randm; $x++) {
   $d2=strtotime("10:21am August 11 2018");			
			$data = array(
