...
class Data_minings_zagov_municipal extends CI_Controller {
/**
...
*/
    ...
    public function index() {
        $arr['page'] = 'data_mining_zagov_municipal';
		//$arr['data_minings_zagov_municipal'] = $this->data_mining->get_all_data_minings_zagov_municipal();
		$arr['user_type'] = $this->session->userdata('user_type');
		$arr['user_id'] = $this->session->userdata('id');
        $this->load->view('admin/data_mining/data_minings_list_zagov_municipal',$arr); 
    }
	
	public function spreadsheet_view(){
		$provinces = $this->data_mining->get_all_provinces('za');
			foreach($provinces as $province){
			$data['type']= 'zagov_municipal';
			$data['title'] = 'ZA Government Municipalities - '.$province->name;
			$data['company'] = $this->data_mining->get_all_mined_companies('zagov_municipal',$province->id);
	...
	public function get_ajax_data() {
		...
		$sql.=" FROM mined_companies_zagov_municipal";
		$query=mysqli_query($conn, $sql) or die("admin/data_mining_zagov_municipal/get_ajax_data: get mined_companies");
		$totalData = mysqli_num_rows($query);
		$totalFiltered = $totalData;  // when there is no search parameter then total number rows = total number filtered rows.
		
		
		$sql = "SELECT id,name,address,telephone,email,category_id,updated_by ";
		$sql.=" FROM mined_companies_zagov_municipal WHERE 1=1";
		...
		while( $row=mysqli_fetch_array($query) ) {  // preparing an array
			...
			$nestedData[] = "<a href='".base_url()."admin/data_mining/data_minings_zagov_municipal/manage_data_mining/".$row["id"]."'>".$row["name"]."</a>";
	...
	public function new_data_mining($next=1) {
		...
		if($this->input->post('proceed'))
		{
			...
			//store company urls
			//local
			$url = "http://www.govpage.co.za/municipal.html";
			...
			$urlArr = $this->data_mining->get_all_companies('zagov_municipal');
			for($x=0;$x<sizeof($urlArr);$x++){
				$url = 'http://www.govpage.co.za'.$urlArr[$x]->url;
				$url = $this->data_mining->getProxyUrl($url,$proxy_url,$proxy_port,$proxy_user,$proxy_pass);
				$co_page = $this->trawl_pages2($url,$category,$urlArr[$x]->name,$urlArr[$x]->gov_type_id,$urlArr[$x]->province_id);
				//echo '<br>';
			}
			
			//now get from all company pages mined - second links
			$urlArr = $this->data_mining->get_all_companies('zagov_municipal2');
			for($x=0;$x<sizeof($urlArr);$x++){
				$url = 'http://www.govpage.co.za'.$urlArr[$x]->url;
				$url = $this->data_mining->getProxyUrl($url,$proxy_url,$proxy_port,$proxy_user,$proxy_pass);
				$co_page = $this->trawl_company_page($url,$category_id,$urlArr[$x]->name,$urlArr[$x]->gov_type_id,$urlArr[$x]->province_id);
				//echo '<br>';
			}
			
			//$this->data_mining->url_link_truncate('zagov_municipal');
		...
		$this->load->view('admin/data_mining/new_data_mining_zagov_municipal',$data);
	}
	...
	function trawl_pages($category,$url,$gov_type){
				...
				foreach($html->find('div[id=wsite-content]') as $result2)
				{
					...
					//{
						foreach ($result2->find('a[href]') as $node)
						{
							$p++;
							if($p==7) $prov_id=8;
							elseif($p==8) $prov_id=7;
							else $prov_id = $p;
							$this->data_mining->url_link_build($node->href,$category,'zagov_municipal',str_replace('&nbsp;',' ',ltrim(strip_tags($node->innertext))),0,$gov_type,$prov_id);
						}
					//}
				}
		...
		function trawl_pages2($url,$category,$name,$gov_type_id,$prov_id){
				...
				foreach($html->find('div[id=wsite-content]') as $result2)
				{
					//foreach($result2->find('div[class=wsite-multicol-col]') as $result3)
					//{
						foreach ($result2->find('a[href]') as $node)
						{
							$name = str_replace("-"," ",$node->href);
							$name = str_replace("/","",$name);
							$name = str_replace(".html","",$name);
							$name = ucwords($name);
							$this->data_mining->url_link_build($node->href,$category,'zagov_municipal2',str_replace('&nbsp;',' ',ltrim(strip_tags($name))),0,$gov_type_id,$prov_id);
						}
					//}
				}
		...
		function trawl_company_page($url,$category_id,$name,$gov_type_id,$prov_id){
					...
					case 3:
						foreach($html->find('div[id=banner-content]') as $result)
						{
							foreach($result->find('span[class=wsite-headline-paragraph]') as $result2)
							{
								...
								for($x=0;$x<$loop;$x++)
								{
									if ((strpos($data[$x], 'Address:') !== false)||(strpos($data[$x], 'Physical:') !== false)) {
						...
						$company_id = $this->data_mining->store_company($name,$data,'zagov_municipal',0,$gov_type_id);
					break;
				}
		...
		function ajax_check_progress1(){
			$no = $this->data_mining->url_link_check('zagov_municipal');
			echo $no;
		}
		
		function ajax_check_progress2(){
			$no = $this->data_mining->mined_companies_check('zagov_municipal');
			echo $no; 
		}
   ...
   public function manage_data_mining($id=0,$type='Update',$action='') {
		...
		if($this->input->post('Update'))
		{
			...
			{
				if($this->data_mining->update($id,'zagov_municipal'))
				{
		...
		if($type=="Update")
		{
			$data_mining = $this->data_mining->get_data_mining($id,'zagov_municipal');
			$data['mined_company'] = $data_mining[0];
			$data['contacts'] = $this->data_mining->get_contacts($id,'zagov_municipal');
			
			$data['categories'] = $this->data_mining->get_categories($id,'zagov_municipal');
        ...
        $this->load->view('admin/data_mining/manage_data_mining_zagov_municipal',$data);
    }
     ...
     public function contact_delete($id,$type_id) {
		...
		redirect('admin/data_mining/data_minings_zagov_municipal/manage_data_mining/'.$id, 'refresh');
    }
	...
	public function delete($id) {
		...
		redirect('admin/data_mining/data_minings_zagov_municipal', 'refresh');
    }
