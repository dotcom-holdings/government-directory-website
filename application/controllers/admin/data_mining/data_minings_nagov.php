...
class Data_minings_nagov extends CI_Controller {
/**
...
*/
    ...
    public function index() {
        $arr['page'] = 'data_mining_nagov';
		//$arr['data_minings_nagov'] = $this->data_mining->get_all_data_minings_nagov();
		$arr['user_type'] = $this->session->userdata('user_type');
		$arr['user_id'] = $this->session->userdata('id');
        $this->load->view('admin/data_mining/data_minings_list_nagov',$arr); 
    }
	
	public function spreadsheet_view(){
		$gov_types = $this->data_mining->get_all_gov_types('na');
			foreach($gov_types as $gov_type){
			$data['type']= 'nagov';
			$data['title'] = 'Namibia Government - '.$gov_type->name;
			$data['company'] = $this->data_mining->get_all_mined_companies('nagov',0,$gov_type->id);
	...
	public function get_ajax_data() {
		...
		$sql.=" FROM mined_companies_nagov";
		$query=mysqli_query($conn, $sql) or die("admin/data_mining_nagov/get_ajax_data: get mined_companies");
		$totalData = mysqli_num_rows($query);
		$totalFiltered = $totalData;  // when there is no search parameter then total number rows = total number filtered rows.
		
		
		$sql = "SELECT id,name,address,telephone,email,category_id,updated_by ";
		$sql.=" FROM mined_companies_nagov WHERE 1=1";
		...
		while( $row=mysqli_fetch_array($query) ) {  // preparing an array
			...
			$nestedData[] = "<a href='".base_url()."admin/data_mining/data_minings_nagov/manage_data_mining/".$row["id"]."'>".$row["name"]."</a>";
	...
	public function new_data_mining($next=1) {
		...
		if($this->input->post('proceed'))
		{
			...
			//store company urls
			//1
			$url = "http://www.govpage.co.za/namibia-central-government.html";
			$url = $this->data_mining->getProxyUrl($url,$proxy_url,$proxy_port,$proxy_user,$proxy_pass);
			$gov_type = 1;	
			$this->trawl_pages($category,$url,$gov_type);
			//2
			$url = "http://www.govpage.co.za/namibia-regional-government.html";
			$url = $this->data_mining->getProxyUrl($url,$proxy_url,$proxy_port,$proxy_user,$proxy_pass);
			$gov_type = 2;	
			$this->trawl_pages($category,$url,$gov_type);
			//3
			$url = "http://www.govpage.co.za/namibia-local-government.html";
			$url = $this->data_mining->getProxyUrl($url,$proxy_url,$proxy_port,$proxy_user,$proxy_pass);
			$gov_type = 3;	
			$this->trawl_pages($category,$url,$gov_type);
			//4
			$url = "http://www.govpage.co.za/namibia-town-councils.html";
			$url = $this->data_mining->getProxyUrl($url,$proxy_url,$proxy_port,$proxy_user,$proxy_pass);
			$gov_type = 4;	
			$this->trawl_pages($category,$url,$gov_type);
			//5
			$url = "http://www.govpage.co.za/namibia-government-parastatals.html";
			$url = $this->data_mining->getProxyUrl($url,$proxy_url,$proxy_port,$proxy_user,$proxy_pass);
			$gov_type = 5;	
			$this->trawl_pages($category,$url,$gov_type);
			//6
			$url = "http://www.govpage.co.za/namibia-tertiary-institutions.html";
			$url = $this->data_mining->getProxyUrl($url,$proxy_url,$proxy_port,$proxy_user,$proxy_pass);
			$gov_type = 6;	
			$this->trawl_pages($category,$url,$gov_type);

			//now get from all company pages mined - second links
			$urlArr = $this->data_mining->get_all_companies('nagov');
			...
			//$this->data_mining->url_link_truncate('nagov');
		...
		$this->load->view('admin/data_mining/new_data_mining_nagov',$data);
	}
	...
	function trawl_pages($category,$url,$gov_type){
				...
				foreach($html->find('div[id=wsite-content]') as $result2)
				{
					foreach($result2->find('div[class=paragraph]') as $result3)
					{
						...
						{
							$this->data_mining->url_link_build($node->href,$category,'nagov',str_replace('&nbsp;',' ',ltrim(strip_tags($node->innertext))),0,$gov_type);
						}
					}
				}
		...
		function trawl_company_page($url,$category_id,$name,$gov_type_id){
			...
			$html->load($url);
				...
				//organisation
						...
						$company_id = $this->data_mining->store_company($name,$data,'nagov',0,$gov_type_id);
		...
		function ajax_check_progress1(){
			$no = $this->data_mining->url_link_check('nagov');
			echo $no;
		}
		
		function ajax_check_progress2(){
			$no = $this->data_mining->mined_companies_check('nagov');
			echo $no; 
		}
   ...
   public function manage_data_mining($id=0,$type='Update',$action='') {
		...
		if($this->input->post('Update'))
		{
			...
			{
				if($this->data_mining->update($id,'nagov'))
				{
		...
		if($type=="Update")
		{
			$data_mining = $this->data_mining->get_data_mining($id,'nagov');
			$data['mined_company'] = $data_mining[0];
			$data['contacts'] = $this->data_mining->get_contacts($id,'nagov');
			
			$data['categories'] = $this->data_mining->get_categories($id,'nagov');
        ...
        $this->load->view('admin/data_mining/manage_data_mining_nagov',$data);
    }
     ...
     public function contact_delete($id,$type_id) {
		...
		redirect('admin/data_mining/data_minings_nagov/manage_data_mining/'.$id, 'refresh');
    }
	...
	public function delete($id) {
		...
		redirect('admin/data_mining/data_minings_nagov', 'refresh');
    }
