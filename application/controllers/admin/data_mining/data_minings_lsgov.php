...
*/
	...
	public function spreadsheet_view(){
		$gov_types = $this->data_mining->get_all_gov_types('ls');
			...
			$data['type']= 'lsgov';
			$data['title'] = 'Lesotho Government - '.$gov_type->name;
			$data['company'] = $this->data_mining->get_all_mined_companies('lsgov',0,$gov_type->id);
	...
	public function new_data_mining($next=1) {
		...
		if($this->input->post('proceed'))
		{
			...
			//store company urls
			//1
			$url = "http://www.govpage.co.za/lesotho-government-ministries.html";
			$url = $this->data_mining->getProxyUrl($url,$proxy_url,$proxy_port,$proxy_user,$proxy_pass);
			$gov_type = 1;	
			$this->trawl_pages($category,$url,$gov_type);
			//2
			$url = "http://www.govpage.co.za/lesotho-district-authorities.html";
			$url = $this->data_mining->getProxyUrl($url,$proxy_url,$proxy_port,$proxy_user,$proxy_pass);
			$gov_type = 2;	
			$this->trawl_pages($category,$url,$gov_type);
			//3
			$url = "http://www.govpage.co.za/lesotho-government-constitutional-bodies.html";
			$url = $this->data_mining->getProxyUrl($url,$proxy_url,$proxy_port,$proxy_user,$proxy_pass);
			$gov_type = 3;	
			$this->trawl_pages($category,$url,$gov_type);
			//4
			$url = "http://www.govpage.co.za/lesotho-regulatory-bodies.html";
			$url = $this->data_mining->getProxyUrl($url,$proxy_url,$proxy_port,$proxy_user,$proxy_pass);
			$gov_type = 4;	
			$this->trawl_pages($category,$url,$gov_type);
			//5
			$url = "http://www.govpage.co.za/lesotho-government-parastatals.html";
			$url = $this->data_mining->getProxyUrl($url,$proxy_url,$proxy_port,$proxy_user,$proxy_pass);
			$gov_type = 5;	
			$this->trawl_pages($category,$url,$gov_type);
			//6
			$url = "http://www.govpage.co.za/lesotho-tertiary-institutions.html";
			$url = $this->data_mining->getProxyUrl($url,$proxy_url,$proxy_port,$proxy_user,$proxy_pass);
			$gov_type = 6;	
			$this->trawl_pages($category,$url,$gov_type);
			...
			//$this->data_mining->url_link_truncate('lsgov');
	...
	function trawl_pages($category,$url,$gov_type){
				...
				foreach($html->find('div[id=wsite-content]') as $result2)
				{
					foreach($result2->find('div[class=paragraph]') as $result3)
					{
						...
						{
							$this->data_mining->url_link_build($node->href,$category,'lsgov',str_replace('&nbsp;',' ',ltrim(strip_tags($node->innertext))),0,$gov_type);
						}
					}
				}
		...
		function trawl_company_page($url,$category_id,$name,$gov_type_id){
			...
			$html->load($url);
				...
				//organisation
						...
						$company_id = $this->data_mining->store_company($name,$data,'lsgov',0,$gov_type_id);
