...
*/
	...
	public function spreadsheet_view(){
		$gov_types = $this->data_mining->get_all_gov_types('bw');
			foreach($gov_types as $gov_type){
			$data['type']= 'bwgov';
			$data['title'] = 'Botswana Government - '.$gov_type->name;
			$data['company'] = $this->data_mining->get_all_mined_companies('bwgov',0,$gov_type->id);
	...
	public function new_data_mining($next=1) {
		...
		if($this->input->post('proceed'))
		{
			...
			//store company urls
			//1
			/*$url = "http://www.govpage.co.za/botswana-government-ministries.html";
			$url = $this->data_mining->getProxyUrl($url,$proxy_url,$proxy_port,$proxy_user,$proxy_pass);
			$gov_type = 1;	
			$this->trawl_pages($category,$url,$gov_type);
			//2
			$url = "http://www.govpage.co.za/botswana-local-authorities.html";
			$url = $this->data_mining->getProxyUrl($url,$proxy_url,$proxy_port,$proxy_user,$proxy_pass);
			$gov_type = 2;	
			$this->trawl_pages($category,$url,$gov_type);
			//3
			$url = "http://www.govpage.co.za/botswana-government-parastatals.html";
			$url = $this->data_mining->getProxyUrl($url,$proxy_url,$proxy_port,$proxy_user,$proxy_pass);
			$gov_type = 3;	
			$this->trawl_pages($category,$url,$gov_type);
			//4
			$url = "http://www.govpage.co.za/botswana-tertiary-institutions.html";
			$url = $this->data_mining->getProxyUrl($url,$proxy_url,$proxy_port,$proxy_user,$proxy_pass);
			$gov_type = 4;	
			$this->trawl_pages($category,$url,$gov_type);
			*/
	...
	function trawl_pages($category,$url,$gov_type){
				...
				foreach($html->find('div[id=wsite-content]') as $result2)
				{
					foreach($result2->find('div[class=paragraph]') as $result3)
					{
						...
						{
							$this->data_mining->url_link_build($node->href,$category,'bwgov',str_replace('&nbsp;',' ',ltrim(strip_tags($node->innertext))),0,$gov_type);
						}
					}
				}
