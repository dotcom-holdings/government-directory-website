...
class Data_minings_szgov extends CI_Controller {
/**
...
*/
    ...
    public function index() {
        $arr['page'] = 'data_mining_szgov';
		//$arr['data_minings_szgov'] = $this->data_mining->get_all_data_minings_szgov();
		$arr['user_type'] = $this->session->userdata('user_type');
		$arr['user_id'] = $this->session->userdata('id');
        $this->load->view('admin/data_mining/data_minings_list_szgov',$arr); 
    }
	
	public function spreadsheet_view(){
		$gov_types = $this->data_mining->get_all_gov_types('sz');
			foreach($gov_types as $gov_type){
			$data['type']= 'szgov';
			$data['title'] = 'Swaziland Government - '.$gov_type->name;
			$data['company'] = $this->data_mining->get_all_mined_companies('szgov',0,$gov_type->id);
	...
	public function get_ajax_data() {
		...
		$sql.=" FROM mined_companies_szgov";
		$query=mysqli_query($conn, $sql) or die("admin/data_mining_szgov/get_ajax_data: get mined_companies_1");
		...
		$query=mysqli_query($conn, $sql) or die("admin/data_mining_szgov/get_ajax_data: get mined_companies_2");
		$totalFiltered = mysqli_num_rows($query); // when there is a search parameter then we have to modify total number filtered rows as per search result. 
		$sql.=" ORDER BY ". $columns[$requestData['order'][0]['column']]."   ".$requestData['order'][0]['dir']."  LIMIT ".$requestData['start']." ,".$requestData['length']." ";
		/* $requestData['order'][0]['column'] contains colmun index, $requestData['order'][0]['dir'] contains order such as asc/desc  */	
		$query=mysqli_query($conn, $sql) or die("admin/data_mining_szgov/get_ajax_data: get mined_companies_3");
		
		$data = array();
		while( $row=mysqli_fetch_array($query) ) {  // preparing an array
			$nestedData=array(); 
		
			$nestedData[] = "<a href='".base_url()."admin/data_mining/data_minings_szgov/manage_data_mining/".$row["id"]."'>".$row["name"]."</a>";
	...
	public function new_data_mining($next=1) {
		...
		if($this->input->post('proceed'))
		{
			...
			//store company urls
			//1
			$url = "http://www.govpage.co.za/swaziland-government-ministries.html";
			$url = $this->data_mining->getProxyUrl($url,$proxy_url,$proxy_port,$proxy_user,$proxy_pass);
			$gov_type = 1;	
			$this->trawl_pages($category,$url,$gov_type);
			//2
			$url = "http://www.govpage.co.za/swaziland-government-departments.html";
			$url = $this->data_mining->getProxyUrl($url,$proxy_url,$proxy_port,$proxy_user,$proxy_pass);
			$gov_type = 2;	
			$this->trawl_pages($category,$url,$gov_type);
			//3
			$url = "http://www.govpage.co.za/swaziland-regional-administration.html";
			$url = $this->data_mining->getProxyUrl($url,$proxy_url,$proxy_port,$proxy_user,$proxy_pass);
			$gov_type = 3;	
			$this->trawl_pages($category,$url,$gov_type);
			//4
			$url = "http://www.govpage.co.za/swaziland-municipalities.html";
			$url = $this->data_mining->getProxyUrl($url,$proxy_url,$proxy_port,$proxy_user,$proxy_pass);
			$gov_type = 4;	
			$this->trawl_pages($category,$url,$gov_type);
			//5
			$url = "http://www.govpage.co.za/swaziland-government-parastatals.html";
			$url = $this->data_mining->getProxyUrl($url,$proxy_url,$proxy_port,$proxy_user,$proxy_pass);
			$gov_type = 5;	
			$this->trawl_pages($category,$url,$gov_type);
			//6
			$url = "http://www.govpage.co.za/swaziland-teacher-training-colleges.html";
			$url = $this->data_mining->getProxyUrl($url,$proxy_url,$proxy_port,$proxy_user,$proxy_pass);
			$gov_type = 6;	
			$this->trawl_pages($category,$url,$gov_type);
			//7
			$url = "http://www.govpage.co.za/technical-vocational-education-and-training-tvet.html";
			$url = $this->data_mining->getProxyUrl($url,$proxy_url,$proxy_port,$proxy_user,$proxy_pass);
			$gov_type = 7;	
			$this->trawl_pages($category,$url,$gov_type);
			//8
			$url = "http://www.govpage.co.za/swaziland-skills-centres.html";
			$url = $this->data_mining->getProxyUrl($url,$proxy_url,$proxy_port,$proxy_user,$proxy_pass);
			$gov_type = 8;	
			$this->trawl_pages($category,$url,$gov_type);
			//9
			$url = "http://www.govpage.co.za/swaziland-universities.html";
			$url = $this->data_mining->getProxyUrl($url,$proxy_url,$proxy_port,$proxy_user,$proxy_pass);
			$gov_type = 9;	
			$this->trawl_pages($category,$url,$gov_type);

			//now get from all company pages mined - second links
			$urlArr = $this->data_mining->get_all_companies('szgov');
			...
			//$this->data_mining->url_link_truncate('szgov');
		...
		$this->load->view('admin/data_mining/new_data_mining_szgov',$data);
	}
	...
	function trawl_pages($category,$url,$gov_type){
				...
				{
					foreach($result2->find('div[class=paragraph]') as $result3)
					{
						foreach ($result3->find('a[href]') as $node)
						{
							$this->data_mining->url_link_build($node->href,$category,'szgov',str_replace('&nbsp;',' ',ltrim(strip_tags($node->innertext))),0,$gov_type);
						}
					}
				}
				...
				//organisation
						foreach($html->find('div[id=banner-content]') as $result)
						{
							foreach($result->find('span[class=wsite-headline-paragraph]') as $result2)
							{
								...
								for($x=0;$x<$loop;$x++)
								{
									if ((strpos($data[$x], 'Address:') !== false)) {
											$address = explode(':',ltrim(strip_tags($data[$x])));
											$address = $address[1];
									}
									elseif (strpos($data[$x], 'Physical:') !== false) {
									...
									elseif (strpos($data[$x], 'P.O. Box') !== false||strpos($data[$x], 'Private Bag') !== false) {
									...
									elseif (strpos($data[$x], 'Fax:') !== false) {
									...
									elseif (strpos($data[$x], 'CALL CENTRE:') !== false) {
											//do nothing
									}
									elseif (strpos($data[$x], 'Head Office') !== false) {
											//do nothing
									}
									elseif (strpos($data[$x], 'Switch Board') !== false) {
											//do nothing
									}
									elseif (strpos($data[$x], 'Fax2mail') !== false) {
											//do nothing
									}
									else {
										$address = ltrim(strip_tags($data[$x]));
						...
						$company_id = $this->data_mining->store_company($name,$data,'szgov',0,$gov_type_id);
		...
		function ajax_check_progress1(){
			$no = $this->data_mining->url_link_check('szgov');
			echo $no;
		}
		
		function ajax_check_progress2(){
			$no = $this->data_mining->mined_companies_check('szgov');
			echo $no; 
		}
   ...
   public function manage_data_mining($id=0,$type='Update',$action='') {
		...
		if($this->input->post('Update'))
		{
			...
			{
				if($this->data_mining->update($id,'szgov'))
				{
		...
		if($type=="Update")
		{
			$data_mining = $this->data_mining->get_data_mining($id,'szgov');
			$data['mined_company'] = $data_mining[0];
			$data['contacts'] = $this->data_mining->get_contacts($id,'szgov');
			
			$data['categories'] = $this->data_mining->get_categories($id,'szgov');
        ...
        $this->load->view('admin/data_mining/manage_data_mining_szgov',$data);
    }
     ...
     public function contact_delete($id,$type_id) {
		...
		redirect('admin/data_mining/data_minings_szgov/manage_data_mining/'.$id, 'refresh');
    }
	...
	public function delete($id) {
		...
		redirect('admin/data_mining/data_minings_szgov', 'refresh');
    }
